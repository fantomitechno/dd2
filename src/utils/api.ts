...
const getPlayerData = async (wsid: string): Promise<PlayerData | undefined> => {
  const dataPb = await getPlayerPb(wsid);

  if (dataPb == null) return undefined;
  ...
  const connected =
    dataLive.last_5_points.length != 0 &&
    Date.now() - dataLive.last_5_points[0]![1]! * 1000 < 10 * 60 * 1000;
